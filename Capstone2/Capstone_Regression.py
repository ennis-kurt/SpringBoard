...
# %% [markdown]
# **The Variables in the agroclimatic data set are described in the following table. Only the ones written in bold text are used in the regression model.**

# %% [markdown]
# <table>
# <tbody><tr><th colspan="3">MAIN VARIABLES</th>
# </tr><tr><th >Name</th><th>Units</th><th >Description</th></tr>
#     
#     <tr><td><b>Biologically effective degree days</b></td><td >°C</td><td>Sum of daily mean temperatures (TG) above 10°C and less than 30°C, over 10 days.</td></tr>
#     <tr><td><b>Cold spell duration index</b></td><td>day</td><td>Number of days per season with at least 6 consecutive days when TN &lt; TN<sup>10th,</sup> where TN is the daily minimum temperature and TN<sup>10th</sup> is the calendar day 10th percentile. To calculate TN<sup>10th</sup> for each day, a 5-day window centered on each equivalent day in the 1981-2010 period is used to improve the statistical robustness from which the 10th percentile of the data is selected  (e.g. for all 3rd of July days between 1981-2010, the 1st, 2nd, 3rd, 4th, 5th July are selected in the 5-day window). This indicator provides information on reduced blossom formation or reduced growth.</td></tr>
#     <tr><td><b>Frost days</b></td><td>day</td><td>Number of days per 10 days when TN &lt; 0°C, where TN is the daily minimum temperature. This indicator provides information on frost damage.</td></tr>
#     <tr><td><b>Growing season length</b></td><td>day</td><td>Number of days between the first occurrence after 1st January (1st July in southern hemisphere) of at least 6 consecutive days with TG &gt; 5°C and the first occurrence after 1st July (1st January in southern hemisphere) of at least 6 consecutive days with TG &lt; 5°C, where TG is the daily mean temperature. This indicator provides an indication whether or not a crop, or a combination of crops, can be sown and subsequently reach maturity within a certain time frame.</td></tr>
#     <tr><td><b>Heavy precipitation days</b></td><td>day</td><td>Number of days per 10 days when RR &gt; 10mm, where RR is the daily precipitation sum. This indicator provides information on crop damage and runoff losses.</td></tr>
#     <tr><td><b>Ice days</b></td><td>day</td><td>Number of days per 10 days when TX &lt; 0°C, where TX is the daily maximum temperature. This indicator provides information on frost damage.</td></tr>
#     <tr><td><b>Maximum number of consecutive dry days</b></td><td>day</td><td>Longest period of consecutive days when RR &lt; 1mm, where RR is the daily precipitation sum. This indicator is used for drought monitoring.</td></tr>
#     <tr><td><b>Maximum number of consecutive frost days</b></td><td>day</td><td>Longest period of consecutive days when TN &lt; 0°C, where TN is the daily minimum temperature. This indicator is used as a general frost damage indicator.</td></tr>
#     <tr><td><b>Maximum number of consecutive summer days</b></td><td>day</td><td>Longest period of consecutive days when TX &gt; 25°C, where TX is the daily maximum temperature. This indicator provides information on drought stress or on optimal growth for C4 crops (crops that use the C4 carbon fixation pathway, e.g. maize).</td></tr>
#     <tr><td><b>Maximum number of consecutive wet days</b></td><td>day</td><td>Longest period of consecutive days when RR &gt; 1mm, where RR is the daily precipitation sum. This indicator provides information on drought, oxygen stress and crop growth (i.e. less radiation interception during rainy days).</td></tr>
#     <tr><td class="variables-name"><b>Mean of daily maximum temperature</b></td><td class="variables-units">K</td><td class="variables-description">Mean value of TX over 10 days, where TX is the daily maximum temperature. This indicator provides information on long-term climate variability and change.</td></tr>
#     <tr><td class="variables-name"><b>Mean of daily mean temperature</b></td><td class="variables-units">K</td><td class="variables-description">Mean value of TG over 10 days, where TG is the daily mean temperature. This indicator provides information on long-term climate variability and change.</td></tr>
#     <tr><td class="variables-name"><b>Mean of diurnal temperature range</b></td><td class="variables-units">°C</td><td class="variables-description">Mean value of the daily difference between TX and TN (TX-TN) over 10 days, where TX and TN are daily maximum and minimum temperature respectively. This indicator provides information on climate variability and change. It also serves as a proxy for information on the clarity (transmittance) of the atmosphere.</td></tr>
#     <tr><td class="variables-name"><b>Precipitation sum</b></td><td class="variables-units">mm</td><td class="variables-description">Sum of RR over 10 days, where RR is the daily precipitation sum. This indicator provides information on possible water shortage or excess.</td></tr>
#     <tr><td class="variables-name"><b>Summer days</b></td><td class="variables-units">day</td><td class="variables-description">Number of days per 10 days when TX &gt; 25°C, where TX is the daily maximum temperature. This indicator provides an indication of the occurrence of heat stress.</td></tr>
#     <tr><td class="variables-name"><b>Tropical nights</b></td><td class="variables-units">day</td><td class="variables-description">Number of days per 10 days when TN &gt; 20°C, where TN is the daily minimum temperature. This indicator provides an indication of occurrence of various pests.</td></tr>
#     <tr><td class="variables-name"><b>Very heavy precipitation days</b></td><td class="variables-units">day</td><td class="variables-description">Number of days per 10 days when RR &gt; 20mm, where RR is the daily precipitation sum. This indicator provides information on crop damage and runoff losses.</td></tr>
#     <tr><td class="variables-name"><b>Warm and wet days</b></td><td class="variables-units">day</td><td class="variables-description">Number of days per 10 days when TG &gt; TG<sup>75th</sup> and RR &gt; RR<sup>75th;</sup> where TG is the daily mean temperature, TG<sup>75th</sup> is the calendar day 75th percentile, RR is the daily precipitation sum and RR<sup>75th</sup> is the 75<sup>th</sup> percentile of precipitation on wet days. To calculate TG<sup>75th</sup> and RR<sup>75</sup> for each day, a 5-day window centered on each equivalent day in the 1981-2010 period is used to improve the statistical robustness from which the 75th percentile of the data is selected (e.g. for all the 3rd of July days between 1981-2010, the 1st, 2nd, 3rd, 4th, 5th July are selected in the 5-day window). This indicator provides an indication of occurrence of various pests and the crop development, especially leaf formation.</td></tr>
#     <tr><td class="variables-name"><b>Warm spell duration index</b></td><td class="variables-units">day</td><td class="variables-description">Number of days per season with at least 6 consecutive days when TX &gt; TX<sup>90th,</sup> where TX is the daily maximum temperature and TX<sup>90th</sup> is the calendar day 90th percentile. To calculate TX<sup>90th</sup> for each day, a 5-day window centered on each equivalent day in the 1981-2010 period is used to improve the statistical robustness from which the 90<sup>th</sup> percentile of the data is selected (e.g. for all the 3rd of July days between 1981-2010, the 1st, 2nd, 3rd, 4th, 5th July are selected in the 5-day window). This indicator provides an indication of the occurrence of heat stress.</td></tr>
#     <tr><td class="variables-name"><b>Wet days</b></td><td class="variables-units">day</td><td class="variables-description">Number of days per 10 days when RR &gt; 1mm, where RR is the daily precipitation sum. This indicator provides information on intercepted reduction.</td></tr>
# </tbody></table>

# %%
vars = ['BEDD', 'CSDI', 'FD', 'GSL', 'R10mm', 
...
agroclim2 = xarr.open_mfdataset(data_dir2+'*.nc', 
                          combine='by_coords', engine='netcdf4')
agroclim2.info()
...
# #### Seasonal variables
# <div class="span5 alert alert-info">
# agroclim2 dataset have 2 spatial, one time coordinates and 15 variables. Now let's see what each of these variables are.
#     </div>

# %%
explore_ds(agroclim2)
...
# %% [markdown]
# <div class="span5 alert alert-info">
# We are only interested some of these columns. Let's drop the ones we will not use. <b>We do not need constant value columns</b>
#     </div>
...
# Now that we have crop data ready for analysis, we should now prepare the climate data. </div>
#
...
# %% [markdown]
# Here is a function that plots the specified crops' and state's time series data
...
# %% [markdown]
# ## Dealing with Trends
# * There is a clear trend in the yield of the most of the crop types except for Non-Irrigated Alfala
# * Significant trend in yield is perhaps related to development in agricultral technologies
...
# * The variances are significanly stronger on non-irrigated crops comparing to their irrigated counterparts. This is probably because the irrigated crops better tolarete fluctuations in climate.
#
# <div class="span5 alert alert-info">
...
#     <p>Therofore, I will first detrend the predictor (climate) variables and response (yield).</p>
#     </div>
...
#     <li>Using cell_methods for each variable seasonal, annual aggregations will be calculated.</li> 
...
agroclim_resample = agroclim.resample(time="Q-DEC", 
                                      loffset='16d', keep_attrs=True) # offset to match with agroclim2
...
#     print(var, agroclim[var].cell_methods)
    # resampling based on cell method
...
clim_seasonal = xarr.merge(dt_arrays_to_merge)

# %% [markdown]
# ### Merging seasonal datasets
# Now that we have resampled our data set seasonally we can merge the resampled dataset to agroclim2 which contains variables with seasonal temporal resolution originally.

# %%
# slicing over time to match with agroclim2 time index
clim_seasonal = clim_seasonal.sel(time=slice("1981-01-16", "2010-10-16"))
climate = xarr.merge([clim_seasonal, agroclim2])
# climate = agroclim2.combine_first(clim_seasonal)

# %% [markdown]
# ## Calculating the climotological state means 
#
# * Define a function to calculate state-wise spatial means. 
#     * This will reduce our space-time data to a simple time-series for each state.
# * Groupby seasons, and merge with agroclim3, the dataset yearly growing degree days
#     * This will create a dataset with only annual variables. Our seasonal groups will be an independent feature
# ### Curse of dimensionality 
# At the end there will be more features than samples. This is a problem because machine learning algorithms assumes P < n, (P: predictors, n: number of samples). Since the purpose of this study is to built an exploratry model rather than predictive model, the ultimate goal would be finding the most important features that effect crop yield. Some variables such as  The following approach may be applied to reduce the number of features
#
# * convert Xarray DataSets to pandas DataFrames. A dataframe for each state
#
...
def plot_roi_random_day(dt, var, state=list(df_crop.State.unique()), _plot=False):
    ...
    crop_states = list(df_crop.State.unique())
    ...
    if _plot:
        ...
        state_coord.apply(lambda x: ax.annotate(text=x.NAME, xy=x.geometry.centroid.coords[0], ha='center', fontsize=7),axis=1);

        # Let's plot the daily average tempreture on a random time
        randm_day = state_clim[var].isel( time=np.random.randint(len(state_clim.time)))
...
# %% [markdown]
# Let's write a function for plotting only a certain month in each year. We will use each month group as a single feature in the model.
...
# %% tags=["Time", "Series", "Step1"] slideshow={"slide_type": "subslide"}
crop_trend_plot("CORN, GRAIN", 'YIELD')
